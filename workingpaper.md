# Working Paper / Mission Briefing fÃ¼r Gemini 3
...
Deine Aufgabe ist es, die UI/UX der Sparkfined PWA basierend auf der neuen Architektur (`docs/UI_UX_ARCHITECTURE.md`) vollstÃ¤ndig zu Ã¼berarbeiten und fertigzustellen. Ein kritischer Aspekt ist dabei die Einbeziehung des Backends (Data Layer), um eine robuste, zukunftssichere Anwendung zu gewÃ¤hrleisten.

Aktuell basiert die App auf einer rudimentÃ¤ren `localStorage` Implementierung. Diese muss professionalisiert und abstrahiert werden, um "Backend-Ready" zu sein (vorbereitet fÃ¼r Supabase Sync).
...
*   `docs/TECHNICAL_SYSTEM_DOCUMENTATION.md` - Der aktuelle technische Stand und DatenflÃ¼sse.
...
Das "Backend" ist in diesem Kontext als **abstrakte Data-Service-Schicht** zu verstehen, die aktuell lokal persistiert, aber asynchrone Schnittstellen fÃ¼r eine echte Remote-DB (Supabase) bereitstellt.
...
*   Nutze `React Query` (TanStack Query) oder `swr` fÃ¼r Data Fetching, anstatt rohe `useEffect` Hooks, falls externe Datenanbindung (Supabase) aktiviert wird. Falls rein lokal: Bleibe bei `useSyncExternalStore`, aber kapsele die Logik sauberer.
...
*   **Typography**: Inter fÃ¼r UI, Monospace fÃ¼r Zahlen.

### 4.2 Navigation & Shell (Prio 1)
*   Implementiere die **Tab-Architektur** fÃ¼r `Settings` und `Journal` (siehe `UI_UX_ARCHITECTURE.md`).
...
## 5. AusfÃ¼hrungsplan (Execution Roadmap)
...
2.  **Types**: Stelle sicher, dass alle EntitÃ¤ten (`Trade`, `Alert`, `WatchlistItem`) in `src/types/` oder Feature-Ordnern sauber definiert sind (Zod Schemas!).

### Phase 2: Navigation & Shell Overhaul
1.  Ãœberarbeite `AppShell.tsx`: Implementiere das responsive Layout strikt nach Design-Vorgabe.
...
*   [ ] App lÃ¤uft fehlerfrei im Dev-Mode.
*   [ ] Keine Linter-Errors (`npm run lint`).
*   [ ] Mobile View ist vollstÃ¤ndig nutzbar (kein horizontales Scrollen, Buttons erreichbar).
