...
export default function PlaybookCard({
  ...
  const btn = "rounded border border-smoke-lighter px-2 py-1 text-xs hover:bg-smoke-light";
    ...
    <div className="rounded-xl border border-smoke-light bg-smoke/40 p-3 text-xs text-mist">
      ...
      <div className="mb-2 grid grid-cols-2 gap-2 md:grid-cols-3">
        ...
        <div className="flex items-center gap-2">
          <span className="text-ash">Entry</span>
          <span className="rounded border border-smoke-light bg-black/30 px-2 py-1">{entry ?? "—"}</span>
          <span className="text-ash">ATR</span>
          <span className="rounded border border-smoke-light bg-black/30 px-2 py-1">{atr ?? "—"}</span>
        </div>
      </div>
      {preview ? (
        <div className="rounded border border-spark/60 bg-spark/20 p-2 text-spark">
          Stop @ <b>{preview.stopPrice.toFixed(6)}</b> · Size ≈ <b>{preview.sizeUnits.toFixed(2)}</b>u · Risk ≈ <b>{preview.riskAmount.toFixed(2)}</b>  
          <div className="mt-1 text-spark">Targets: {preview.rrTargets.map((t,i)=>`${preview.rrList[i]}R→${t.toFixed(6)}`).join(" · ")}</div>
          <div className="text-[11px] text-spark/80">Kelly-lite ~ {preview.kellyLitePct.toFixed(2)}% (Info)</div>
        </div>
      ) : (
        <div className="rounded border border-smoke-light bg-black/30 p-2 text-fog">Entry & ATR nötig für Vorschau.</div>
      )}
      <div className="mt-2">
