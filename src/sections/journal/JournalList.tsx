  ...
  return (
    ...
    >
      ...
      {note.screenshotDataUrl && (
        <img src={note.screenshotDataUrl} alt="" className="mt-3 h-28 w-full rounded object-cover" />
      )}
      ...
      <dl className="mt-3 grid grid-cols-2 gap-2 text-[11px] text-zinc-400">
        ...
        <div>
          ...
          <dd className="mt-0.5">
            {entryPriceDisplay(entryPrice)} → {entryPriceDisplay(exitPrice)}
          </dd>
        </div>
        <div>
          <dt className="text-[10px] uppercase tracking-wide text-zinc-500">Size</dt>
          <dd className="mt-0.5">{valueDisplay(positionSize)}</dd>
        </div>
        <div>
          <dt className="text-[10px] uppercase tracking-wide text-zinc-500">SL / TP</dt>
          <dd className="mt-0.5">
            {valueDisplay(stopLoss)} / {valueDisplay(takeProfit)}
          </dd>
        </div>
      </dl>
...
function valueDisplay(value?: number) {
  return typeof value === "number" && Number.isFinite(value) ? value.toString() : "—";
}

function entryPriceDisplay(value?: number) {
