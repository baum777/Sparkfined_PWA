...
export default function AlertEditDialog({ alert }: AlertEditDialogProps) {
  const updateAlert = useAlertsStore((state) => state.updateAlert);
  const [isOpen, setIsOpen] = React.useState(false);
  const [formState, setFormState] = React.useState<FormState>(() => hydrateFromAlert(alert));
  const [errors, setErrors] = React.useState<FormErrors>(() => getDefaultErrors());
  const [isSubmitting, setIsSubmitting] = React.useState(false);
  const dialogTitleId = React.useId();

  const buildFormState = React.useCallback(() => hydrateFromAlert(alert), [alert]);
      ...
      {isOpen ? (
        ...
        >
          <form
            ...
            data-testid="alert-edit-form"
          >
            <header className="space-y-1">
              <p className="text-xs uppercase tracking-wide text-text-tertiary">Edit alert</p>
              <h2 id={dialogTitleId} className="text-2xl font-semibold text-text-primary">
                Update alert
              </h2>
              <p className="text-sm text-text-secondary">
                Adjust the thresholds or narrative so the automation stays relevant.
              </p>
            </header>

            <div className="space-y-4">
              <div>
                <label className="mb-2 block text-sm font-medium text-text-secondary" htmlFor="alert-edit-symbol-input">
                  Symbol
                </label>
                <Input
                  id="alert-edit-symbol-input"
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-text-secondary" htmlFor="alert-edit-type-select">
                  Type
                </label>
                <Select
                  ...
                  triggerProps={{ 'data-testid': 'alert-edit-type-select', id: 'alert-edit-type-select' }}
                />
              </div>

              <div>
                <label className="mb-2 block text-sm font-medium text-text-secondary" htmlFor="alert-edit-condition-input">
                  Condition
                </label>
                <Input
                  id="alert-edit-condition-input"
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-text-secondary" htmlFor="alert-edit-threshold-input">
                  Threshold
                </label>
                <Input
                  id="alert-edit-threshold-input"
                  type="number"
                  step="any"
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-text-secondary" htmlFor="alert-edit-timeframe-select">
                  Timeframe
                </label>
                <Select
                  ...
                  triggerProps={{ 'data-testid': 'alert-edit-timeframe-select', id: 'alert-edit-timeframe-select' }}
                />
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-3">
              <Button
                ...
                data-testid="alert-edit-cancel-button"
              >
                Cancel
              </Button>
              <Button type="submit" size="sm" isLoading={isSubmitting} data-testid="alert-edit-submit-button">
...
function hydrateFromAlert(alert: Alert): FormState {
  return {
    symbol: alert.symbol,
    type: alert.type,
