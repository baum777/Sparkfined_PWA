// Journal handlers — consolidated from api/journal/*.ts
// Handles CRUD operations and export
...
export async function handleExport(req: Request): Promise<Response> {
  const url = new URL(req.url);
  const userId = getUserId(req);
  const fmt = (url.searchParams.get("fmt") || "json").toLowerCase(); // json|md
  ...
  rows.sort((a, b) => a.createdAt - b.createdAt); // chronological
