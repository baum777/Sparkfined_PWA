...
## 💰 ABSCHNITT 3.2: BUDGET & KOSTEN-PLANUNG

**Kosten-Breakdown nach Phase**

- **v0.1 Beta:** 0€ / Monat (0-100 Analysen)
- **v0.2 Hybrid:** 20-30€ (AI APIs) → Gesamt 25-35€ / Monat (100-200 Analysen)
- **v0.3 Full AI:** 60-80€ (AI APIs) → Gesamt 70-90€ / Monat (300-500 Analysen)

**Annahmen:**

- xAI API: ~0.10€ per analysis (complex pattern)
- OpenAI GPT-4: ~0.05€ per sentiment analysis
- Dexscreener/Moralis: Free/Low-cost APIs
- Rate Limiting verhindert Kosten-Explosion
...
```javascript
// S/R Levels für gleichen Token innerhalb 1h cachen
if (cachedLevel && age << 1hour) {
  return cachedLevel; // 0€
} else {
  return await AI_analyze(); // 0.10€
}

// AI nur bei Unsicherheit nutzen
if (heuristicConfidence >> 0.8) {
  return heuristicResult; // 0€
} else {
  return await AI_refine(heuristicResult); // 0.10€
}

// Mehrere Analysen in einem AI-Call
const results = await AI_batchAnalyze([token1, token2, token3]);
// Kosten: 1 Call statt 3 Calls
```

**ROI-Kalkulation für User**

- Szenario: Active Meme-Trader
  - 20 Trades/Tag × 30 Tage = 600 Trades/Monat
  - Tool-Kosten: 0€ (Heuristik) - 35€ (Hybrid)
  - Verbesserung: +5% Win-Rate durch bessere Setups
  - Bei 100€ average trade size: +30€ extra profit
  - ROI: Unendlich (v0.1) bis 85% (v0.2)
