...
export default async function handler(req: Request) {
  ...
  try {
    ...
    // --- Early secret checks (no cost, no cache, no provider call) ---

    if (provider === "openai" && !openaiKey) {
      // Test-Erwartung: status 200, ok:false, error enthält "OPENAI_API_KEY missing"
      return json({ ok: false, error: "OPENAI_API_KEY missing" }, 200);
    }

    if (provider === "grok" && !grokKey) {
      // Symmetrisches Verhalten für Grok
      return json({ ok: false, error: "GROK_API_KEY missing" }, 200);
    }

    const prompt = templateId
      ? render(templateId, vars || {})
      : { system, user };

    if (!prompt.user) {
      return json(
        { ok: false, error: "user or templateId required" },
        400
      );
    }
    ...
    if (cached) {
      return json({ ok: true, fromCache: true, ...cached }, 200);
    }
...
function ensureAiProxyAuthorized(req: Request): Response | null {
  ...
  if (!secret) {
    if (!isProd) {
      console.warn(
        "[ai/assist] AI_PROXY_SECRET not set – allowing request in non-production environment"
      );
      return null;
    }
    console.error(
      "[ai/assist] AI_PROXY_SECRET missing – blocking AI proxy request"
    );
  ...
  const T: any = {
    "v1/analyze_bullets": (v: any) => ({
      ...
      user: [
        ...
        `- volatility24hσ=${
          v.metrics ? (v.metrics.volStdev * 100).toFixed(2) : "n/a"
        }%`,
        ...
        (v.matrixRows || [])
          .map((r: any) =>
            `${r.id}: ${r.values
              .map((s: number) => (s > 0 ? "Bull" : s < 0 ? "Bear" : "Flat"))
              .join(", ")}`
          )
          .join("\n"),
